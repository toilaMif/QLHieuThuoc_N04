...
public class frm_default3 extends JFrame implements ActionListener {
	...
	public frm_default3() {
		...
		jlTieuDeTrang.setForeground(Color.white);
		...
		btnXoaRong = createButton("❌ Thoát", Color.RED);
		...
		anh = new JLabel();
		khungAnh.addComponentListener(new ComponentAdapter() {
		    @Override
		    public void componentResized(ComponentEvent e) {
		        int maxWidth = khungAnh.getWidth() * 80 / 100;
		        int maxHeight = khungAnh.getHeight() * 80 / 100;

		        

		        // Xác định kích thước phù hợp với tỷ lệ 3x4
		        ...
		        if (width > 0 && height > 0) {
		            anh.setPreferredSize(new Dimension(width, height));
...
//        thongtin.add(btnNewButton, BorderLayout.SOUTH);
	...
	protected HashMap<String, JTextField> fieldMap = new HashMap<>(); // Lưu theo tên
	...
	public void taoThongTin() {
	    ...
	    for (int i = 0; i < tieuDeCot.length; i++) {
	        ...
	        textField.setBorder(BorderFactory.createLineBorder(new Color(180, 220, 85), 1));

	        String fieldName = tieuDe.replace(" ", "").toLowerCase();
	        textField.setName(fieldName); // Đặt tên
	        fieldMap.put(fieldName, textField); // Lưu vào HashMap
